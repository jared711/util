function [rvdot] = CCP(t, rv, mu, mu3, a3, theta0)
%CCP Concentric Circular Problem set in the frame of the large body and one
%small body (e.g. Jupiter-Europa barycenter, with Ganymede in motion)
% 
% [rvdot] = CCP(t, rv, mu, mu3, a3, theta0)
% 
...
% 
% See also: BCP1 BCP2 CR3BP PCR3BP

% Author: Jared Blanchard 	Date: 2022/10/07 11:12:40 	Revision: 0.1 $

if nargin < 6;  theta0 = 0; end

n3 = sqrt((1+mu3)/a3^3);
theta = theta0 + (n3-1)*t;% theta should be positive for the moon in motion
...
rvdot = zeros(6,1);
rvdot(1:3) = rv(4:6);
rvdot(4) = -((1-mu)*(x+mu)/r13 + mu*(x-1+mu)/r23 + mu3*(x-x3)/r33) + x + 2*ydot;
rvdot(5) = -((1-mu)*y     /r13 + mu*y       /r23 + mu3*(y-y3)/r33) + y - 2*xdot;
