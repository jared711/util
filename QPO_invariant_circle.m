function [U, rho, eig_idx] = QPO_invariant_circle(rv0, T0, N, alpha)
%QPO_INVARIANT_CIRCLE computes invariant circle approximation from center
%manifold of periodic orbit
% 
% [U, rho, eig_idx] = QPO_INVARIANT_CIRCLE(rv0, T0, N, alpha)
% 
...
% 
% Outputs:  U (Nx6) [NON] matrix of points on invariant Circle 
...
%     u(:,i) = rv0 + alpha*(cos(theta(i))*real(Veig(:,eig_idx)) - sin(theta(i))*imag(Veig(:,eig_idx)));
    u(:,i) = alpha*(cos(theta(i))*real(v) - sin(theta(i))*imag(v));
end
U = u';
